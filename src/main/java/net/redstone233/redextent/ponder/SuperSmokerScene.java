...
public class SuperSmokerScene {
    ...
    public static void superSmoker(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        for (BlockPos pos : coalPositions) {
            ...
            Vec3 centerPos = util.vector().centerOf(pos);
        ...
        for (BlockPos pos : woodPositions) {
            ...
            scene.effects().emitParticles(
                    centerPos,
                    scene.effects().simpleParticleEmitter(ParticleTypes.CAMPFIRE_COSY_SMOKE, new Vec3(0, 0.1, 0)),
                    2, 1
            );
            scene.idle(3);
        }
        ...
        for (BlockPos pos : coalPositions) {
            ...
            Vec3 centerPos = util.vector().centerOf(pos.getX(), 2, pos.getZ());
            scene.effects().emitParticles(
                    centerPos,
                    scene.effects().simpleParticleEmitter(ParticleTypes.SMOKE, new Vec3(0, 0.1, 0)),
                    2, 1
        ...
        // 显示烟熏炉并添加特效
        Vec3 smokerPos = util.vector().centerOf(3, 2, 3);
        scene.effects().emitParticles(
                smokerPos,
                scene.effects().simpleParticleEmitter(ParticleTypes.CAMPFIRE_SIGNAL_SMOKE, new Vec3(0, 0.2, 0)),
                10, 20
        );
    ...
    public static void init() {
        RedExtendMod.LOGGER.info("SuperSmokerScene init");
    }
}